...
Description: ML-KEM key generation and inspection
Date       : $(date -u)
Hostname   : $(hostname)
...
openssl list -providers -verbose | tee "${RESULTS_DIR}/providers.txt"

echo
echo "[*] Available KEM algorithms..."
openssl list -kem-algorithms | tee "${RESULTS_DIR}/kem-algorithms.txt"

echo
echo "[*] Available TLS groups..."
openssl list -tls-groups | tee "${RESULTS_DIR}/tls-groups.txt"
